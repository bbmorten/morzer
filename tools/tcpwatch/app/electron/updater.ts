...
export async function downloadAndInstallUpdate(
  ...
  try {
    ...
    const script = [
      ...
      'rm -rf "$BACKUP"',
      '',
      '# Move current app to backup (retry up to 5 times)',
      'MOVED=0',
      'for i in $(seq 1 5); do',
      ...
      '    break',
      '  fi',
      '  echo "[$(date)] Retry $i: mv to backup failed, waiting..." >> "$LOG"',
      '  sleep 1',
      'done',
      'if [ "$MOVED" -ne 1 ]; then',
      '  echo "[$(date)] ERROR: Failed to move old app to backup after retries" >> "$LOG"',
      '  rm -rf "$STAGING"',
      '  exit 1',
      'fi',
